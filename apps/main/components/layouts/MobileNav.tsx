  ...
  return (
    <Sheet open={opened} onOpenChange={(open) => setOpened(open)}>
      <SheetTrigger asChild className="block md:hidden">
        <Button
          className="text-pageText px-2 flex justify-center items-center"
          variant="default"
      ...
      <SheetContent
        style={{ zIndex: 1000 }}
        className="w-full  h-full [&>svg]:text-pageText border-0"
      >
        <div className="py-20 w-full flex flex-col absolute left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2 z-10 gap-10 items-center">
          {navlinks.map((navLink) => (
            <div
              // href={navLink.href}
              className="text-pageText hover:text-accentColor"
              key={navLink.title}
              onClick={(e) => {
                e.preventDefault()
