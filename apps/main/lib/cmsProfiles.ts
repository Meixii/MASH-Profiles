...
export type CMSSectionStat = {
  label: string
  value: string
}

export type CMSProjectItem = {
  description: string
...
export type CMSAboutContent = {
  ...
  stats?: CMSSectionStat[]
  title?: string | null
}

export type CMSProjectsContent = {
  body?: string | null
  items?: CMSProjectItem[]
  title?: string | null
}
...
export type CMSProfileContent = {
  about?: CMSAboutContent | null
  blog?: CMSBlogContent | null
  contact?: CMSContactContent | null
  hero?: CMSHeroContent | null
  projects?: CMSProjectsContent | null
}

export type CMSProfileCard = {
  accentColor: string
  ...
  slug: string
  summary?: string | null
  content?: CMSProfileContent | null
...
export type CMSProfilesResponse = {
  profiles: CMSProfileCard[]
  settings: CMSSiteSettings
}

const CMS_BASE_URL =
  process.env.NEXT_PUBLIC_CMS_BASE_URL?.trim() || process.env.CMS_BASE_URL?.trim() || ''
...
export async function getCMSProfiles(): Promise<CMSProfilesResponse | null> {
  ...
  try {
    const response = await fetch(`${CMS_BASE_URL.replace(/\/$/, '')}/api/cms-data/profiles`, {
      next: {
        revalidate: 60,
      },
    })
    ...
    return (await response.json()) as CMSProfilesResponse
  } catch {
    return null
  }
}

export async function getCMSProfileBySlug(
  slug: string
): Promise<{ profile: CMSProfileCard | null; settings: CMSSiteSettings | null }> {
  if (!CMS_BASE_URL) {
    return {
      profile: null,
      settings: null,
  ...
  try {
    const response = await fetch(
      `${CMS_BASE_URL.replace(/\/$/, '')}/api/cms-data/profiles/${encodeURIComponent(slug)}`,
      {
        next: {
          revalidate: 60,
        },
